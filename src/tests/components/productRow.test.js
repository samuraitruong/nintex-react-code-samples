...
describe('ProductRow render tests', () => {
    ...
    it('Should trigger on change event on quantity change', () => {
        ...
        expect(onQuantityUpdate).toBeCalledWith({"id": "123", "name": "test", "price": 100, "quantity": 5});
    })
    ...
    it('Should update price column when quantity update with valid number', () => {
        ...
        input
            .at(0)
            .simulate('change', event);
        expect(onQuantityUpdate).toBeCalledWith({"id": "1", "name": "test", "price": 299.99, "quantity": 10});
